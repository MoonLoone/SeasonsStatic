...
layout(location = 4) in float aFlip;
uniform vec2 uResolution;
uniform float uVisibleCount;
out vec2 vUv; out float vRnd; out float vFade;
void main(){
    // Частицы с индексом за границей видимости гаснут (плавная полоса ~24 шт).
    ...
    float fc = abs(cos(aFlip));
    float scaleX = 0.55 + 0.45 * fc;
    vec2 local = vec2(c.x * scaleX, c.y) * aSize;
